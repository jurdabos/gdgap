-- R2, R3, R9, R11: derived equity measures, InnoDB foil — imputation-share visibility (R3), care-trip
-- share by sex and the PPR representation ratio (R2), small-cell suppression at n >= 30 (R9).
-- Requires MySQL 8.0.19+ (window functions and CTEs in view definitions).
CREATE DATABASE IF NOT EXISTS gdgap_aware;
DROP VIEW IF EXISTS gdgap_aware.v_sex_imputation_share;
CREATE VIEW gdgap_aware.v_sex_imputation_share AS
SELECT
    sex_source,
    count(*) AS persons,
    count(*) * 100.0 / sum(count(*)) OVER () AS share_pct
FROM gdgap_aware.dim_person_sex
WHERE is_current
GROUP BY sex_source;

DROP VIEW IF EXISTS gdgap_aware.v_care_trip_share_by_sex;
CREATE VIEW gdgap_aware.v_care_trip_share_by_sex AS
SELECT
    d.sex_code,
    count(*) AS trips_n,
...
FROM gdgap_aware.fact_trip AS f
JOIN gdgap_aware.dim_person_sex AS d
    ON d.houseid = f.houseid AND d.personid = f.personid AND d.is_current
GROUP BY d.sex_code
HAVING count(*) >= 30;

DROP VIEW IF EXISTS gdgap_aware.v_ppr;
CREATE VIEW gdgap_aware.v_ppr AS
WITH care AS (
    SELECT
        d.sex_code,
...
population AS (
    SELECT
        ...
        count(*) AS person_n
    FROM gdgap_aware.fact_person AS p
    JOIN gdgap_aware.dim_person_sex AS d
